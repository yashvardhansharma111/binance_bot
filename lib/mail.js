...
function getTransporter() {
  ...
  console.log('[mail] SMTP config:', {
    ...
    passFirst4: pass ? pass.slice(0, 4) : '(undefined)',
    passLast4:  pass ? pass.slice(-4)  : '(undefined)',
    from,
    SMTP_PASS_RAW: JSON.stringify(pass),
  });

  if (!host || !port || !user || !pass) {
    const missing = ['SMTP_HOST','SMTP_PORT','SMTP_USER','SMTP_PASS'].filter(k => !process.env[k]);
    throw new Error(`[mail] Missing SMTP env vars: ${missing.join(', ')}`);
  }
...
<body><div class="wrap">
  ...
  <div class="footer">© 2026 TrickyX.ai · support@trickyx.ai<br>This is an automated message, please do not reply.</div>
</div></body></html>`;
}
...
export async function sendOtpEmail(email, otp, purpose) {
  ...
  try {
    ...
    console.error('[mail] sendOtpEmail FAILED:', {
      message:      err.message,
      code:         err.code,
...
export async function sendTradeEmail(email, name, type, trade) {
  ...
  try {
    ...
    console.log('[mail] Trade email sent to', email);
  } catch (err) {
    console.error('[mail] sendTradeEmail FAILED:', {
      message:      err.message,
      code:         err.code,
      response:     err.response,
      responseCode: err.responseCode,
...
export async function sendWelcomeEmail(email, name, referralCode) {
  ...
  try {
    ...
    console.error('[mail] sendWelcomeEmail FAILED:', {
      message:      err.message,
      code:         err.code,
      response:     err.response,
      responseCode: err.responseCode,
